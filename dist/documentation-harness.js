import { createAgentSession, createWriteToolDefinition, DefaultResourceLoader, defineTool, getAgentDir, ModelRuntime, SessionManager, SettingsManager, } from "@earendil-works/pi-coding-agent";
...
export async function runInteractiveDocumentationHarness(initialOptions) {
    ...
    logVerbose(options, `Requested model: ${options.model ? `${options.model.provider}/${options.model.id}` : "(default)"}`);
    ...
    if (options.model && !initialModel) {
        logVerbose(options, "Requested model was not found before extension loading. It will be resolved again after session startup.");
    }
    ...
    logVerbose(options, `Loaded extensions: ${extensionsResult.extensions.length}`);
    ...
    logVerboseList(options, "Models after session startup", modelRuntime.getModels().map((model) => `${model.provider}/${model.id}`));
...
function createDecisionLogTool(decisionLogPath) {
    return defineTool({
        ...
        async execute(_toolCallId, params) {
            ...
            return {
                content: [{ type: "text", text: `Recorded decision in ${decisionLogPath}` }],
                details: undefined,
            };
        },
    });
}
function entryToDecisionLogEntry(entry) {
    return {
...
function createDocumentWriteTool(outputDir) {
...
function documentWriteError(message) {
    return {
...
async function hydrateOptionsFromSessionMetadata(options, sessionManager) {
    ...
    return {
        ...
        referencePaths: providedOptions.has("reference") || providedOptions.has("reference-dir")
...
async function readSessionMetadata(sessionManager) {
...
async function writeSessionMetadata(options, sessionManager) {
    const sessionFile = sessionManager.getSessionFile();
...
async function tryWriteSessionMetadata(options, sessionManager) {
    try {
...
function getSessionMetadataPath(sessionFile) {
...
function describeSessionMode(options) {
    if (options.sessionFile) {
        return "session-file";
    }
    if (options.resume) {
...
function logVerboseProviderDiagnostics(options, modelRuntime) {
    if (!options.verbose) {
        return;
    }
    ...
    for (const providerId of providerIds) {
        ...
        if (config && "models" in config && Array.isArray(config.models)) {
            process.stderr.write(`[verbose] Provider ${providerId} registered config model count: ${config.models.length}\n`);
        }
    }
    if (!options.model) {
        return;
    ...
    if (requestedProviderModels.length === 0 && providerIds.includes(options.model.provider)) {
        process.stderr.write(`[verbose] Requested provider ${options.model.provider} is registered, but it has no models. If the extension discovers models dynamically, fetch and register them in the async extension factory instead of a session_start handler.\n`);
    }
    const requestedModel = modelRuntime.getModel(options.model.provider, options.model.id);
    if (!requestedModel) {
        process.stderr.write(`[verbose] Requested model ${options.model.provider}/${options.model.id} is not registered.\n`);
    }
}
